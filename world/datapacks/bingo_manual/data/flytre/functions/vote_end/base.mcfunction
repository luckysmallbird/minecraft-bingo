...
execute if score votes vote matches 1 run tellraw @a[team=!] ["",{"selector":"@s"},{"text":" has requested to end the game in a draw.","color":"light_purple"},{"text":"\n"},{"text":"Click here to vote to end the game. Vote must be unanimous.","color":"light_purple","clickEvent":{"action":"run_command","value":"/trigger vote_end"}}]
execute if score votes vote matches 1 run scoreboard players set vote_time vote 600

execute if score votes vote matches 2.. run tellraw @a[team=!] ["",{"selector":"@s"},{"text":" has agreed.","color":"light_purple"}]
